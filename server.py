...
TOUT -> refonte complète d'à peut près tout
Utilisation de pages en .py pour la configuration
Orientation du projet autour de la "sécurité"


Quoi de neuf dans la V7 ?
Création d'une fonction "serve" permettant de serve une à plusieurs pages en .py
Les pages serve par "serve" seraient des fonctions qui seraient dans une lisste donnée en argument à "serve"
Et donc, on aurait tout un site situé uniquement dans un seul fichier, utile pour flex en disant "ouais mon site fait 3000 lignes de code"
Il est recomander d'utiliser la fonction prepare pour permettre à une page de renvoyer un fichier se trouvant sur l'ordinateur

Ajout également d'un support du C pour les pages, on pourrait donc créer des pages en .c que le serveur pourait appeler comme si c'était des pages en .py
Evidement, ne codant pas en C, il ce peut que cette idée à la con ne voie jamais le jour

Ajout d'un support php en faisant agir le serveur python comme un proxy pour un serveur appache interne

Ajout d'une classe permettant de gérer les tables d'une db sqlite3 comme un dictionnaire
Sources:
...
function title_carousel(title,length,delay){
    var position = 0;var date=null;var currentDate = null
    while (true){
        document.title = title.substr(position,length);
        position = position + 1;
...
}//Ca n'a rien à faire ici mais voilà le code d'un carousel de titre en js
...
if "-h" in sys.argv:
    print("""
-start "addresse" "port" "root" "log" -> cré un serveur et bind un port
-listen "queue" -> demarre le serveur precedement créé
-exec -> demarre un thread d'éxécution de commande côté serveur
-config -> lance une page web dont l'addresse est localhost:2004 contenant un formulaire pour configurer le serveur
    /!\\ Peut seulement être utilisé seul
    /!\\ Ne devrait pas être utilisé sur un serveur forwardé ou sur un wifi publique
""")
